...
class Trainer:
    ...
    def deal_with_objective(self, pred, truth):
        ...
        if type(self.objective) == nn.CrossEntropyLoss:
            ...
            type(self.objective) == nn.MSELoss
            or type(self.objective) == nn.modules.loss.BCEWithLogitsLoss
            or type(self.objective) == nn.L1Loss
        ):
        ...
        with torch.no_grad():
            ...
            for j in self.test_dataloader:
                ...
                if self.is_packed:
                    ...
                    out = self.model([i.float().float().to(self.device) for i in j[:-1]])
                if (
                    type(self.objective) == torch.nn.modules.loss.BCEWithLogitsLoss
                    or type(self.objective) == torch.nn.MSELoss
                ):
