...
def load_model_and_data(cfg, checkpoint_path, test_data_path, cif_folder):
    ...
    test_dataset = CIFData(
        test_data, cif_folder, cfg.MODEL.INIT_FILE, cfg.MODEL.MAX_NBRS, cfg.MODEL.RADIUS, cfg.SOLVER.RANDOMIZE
    )
...
def main():
    ...
    trainer = pl.Trainer(
        accelerator="gpu" if int(args.devices) > 0 else "cpu", devices=args.devices if int(args.devices) > 0 else None
    )
