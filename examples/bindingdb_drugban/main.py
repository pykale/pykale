...
from kale.pipeline.drugban_lightning_trainer import DrugbanDATrainer, DrugbanTrainer
...
def main():
    ...
    if cfg.COMET.USE:
        logger = CometLogger(
            ...
            save_dir=cfg["RESULT"]["OUTPUT_DIR"],
    ...
    # ---- setup model ----
    if not cfg.DA.USE:
        model = DrugbanTrainer(model=DrugBAN(**cfg), logger=logger, **cfg)
        trainer = pl.Trainer(
            devices="auto",
        ...
        model = DrugbanDATrainer(model=DrugBAN(**cfg), discriminator=Discriminator, logger=logger, **cfg)
        trainer = pl.Trainer(
            devices="auto",
            accelerator="gpu" if torch.cuda.is_available() else "cpu",
            max_epochs=cfg["SOLVER"]["MAX_EPOCH"],
