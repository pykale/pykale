...
from kale.loaddata.tabular_access import load_uncertainty_pairs_csv, apply_confidence_inversion
    ...
    for model in models_to_compare:
        for landmark in landmarks:
            for uncertainty_pairing in uncertainty_error_pairs:
                ...
                landmark_results_path = os.path.join(base_dir, "Results", model, dataset, landmark_base_filename+"_l"+str(landmark)+'.csv')
                print("lm res path: ", landmark_results_path)
                for fold in range(num_folds):
                    fold_info_path = os.path.join(base_dir, "Data", dataset, 'fold_information', fold_file_base+"_fold"+str(fold)+'.json')
                    ...
                    update_csv_w_fold(landmark_results_path, uncertainty_localisation_er, uncertainty_measure, fold_info_path, json_dict)
