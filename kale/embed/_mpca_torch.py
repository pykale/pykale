...
from .mpca import _check_dim_shape
...
class MPCA(nn.Module):
    ...
    Args:
        ...
        mean_remove (bool, optional): Whether remove the mean of input data. Defaults ot True.
            Set to False if the input data mean is zero already .
    ...
    def __init__(self, var_ratio=0.97, max_iter=1, mean_remove=True):
        super().__init__()
        ...
        if max_iter > 0 and isinstance(max_iter, int):
            ...
            error_msg = "Number of max iterations must be an positive integer but given %s" % max_iter
    ...
    def _fit(self, x):
        ...
        if self.mean_remove:
            ...
            self.mean_ = torch.zeros(x[0, :].shape)
        x = x - self.mean_
        ...
        for i_iter in range(self.max_iter):
            ...
            for i in range(1, n_dim):  # ith mode
                ...
                for j in range(n_spl):
                    ...
                    xj_ = multi_mode_dot(
                        x_j,
                        [self.factors[m] for m in range(n_dim - 1) if m != i - 1],
                ...
                self.factors[i - 1] = (eig_vectors[:, idx_sorted][:, : shape_out[i - 1]]).T

        tensor_pc_vec = self.forward(x, reduce_mean=False, return_vector=True, sort_by_var=False)
    ...
    def forward(self, x, reduce_mean=True, return_vector=False,
                sort_by_var=True, n_components=None):
        """Perform dimension reduction on x

        Args:
            x (tensor):
            reduce_mean (bool): Whether reduce the mean estimated from training data. Defaults to True.
            return_vector (bool): Whether return the tensor principal components in vector.
                Defaults to False.
    ...
    def reconstruct(self, x, add_mean=False):
        ...
        if x.ndim <= 2:
            ...
            if n_feat <= all_comp:
                ...
                error_msg = "Feature dimension exceeds the shape upper limit."
                logging.error(error_msg)
                raise ValueError(error_msg)
        ...
        x_rec = multi_mode_dot(x, self.factors, modes=[m for m in range(1, self.n_dim)], transpose=True)
