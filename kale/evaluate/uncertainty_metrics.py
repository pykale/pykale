...
def evaluate_bounds(estimated_bounds, bin_predictions, uncertainty_pairs, num_bins, landmarks, num_folds=8, show_fig=False, verbose=False):
    ...
    for i, (model, data_structs) in enumerate(bin_predictions.items()):
        ...
        for up in uncertainty_pairs:
            ...
            for fold in range(num_folds):
                ...
                fold_errors =  data_structs[(data_structs['Testing Fold'] == fold)][["uid", "landmark", uncertainty_type + " Error"]]
                fold_bins =data_structs[(data_structs['Testing Fold'] == fold)][["uid", "landmark", uncertainty_type + " Uncertainty bins"]]
                fold_bounds =   strip_for_bound(error_bounds[error_bounds['fold']==fold][uncertainty_type+" Uncertainty bounds"].values)

                return_dict = bin_wise_bound_eval(fold_bounds, fold_errors,fold_bins,  landmarks, uncertainty_type,  num_bins=num_bins, show_fig=show_fig)
                ...
                for idx_bin in range(len(return_dict['mean all bins'])):
                    ...
                    fold_learned_bounds_bins_lmsnotsep[idx_bin]= fold_learned_bounds_bins_lmsnotsep[idx_bin] + return_dict['mean all'][idx_bin]

      
        

            if verbose:
                print("\n \n \n OVERALL BOUND ACC STATS FOR ", model + uncertainty_type)
                print("overall mean ACC: ", np.round(np.mean(fold_learned_bounds_bins_lmsnotsep),3), ' +/- ', np.round(np.std(fold_learned_bounds_mean_bins),3))

                print("QUANTILe bin ACC: ",end="")
                for idx_bin in range(num_bins):
                    print("B", idx_bin, np.round(np.mean(fold_learned_bounds_bins_lmsnotsep[idx_bin]),3), '+/-', np.round(np.std(fold_learned_bounds_bins_lmsnotsep[idx_bin]),3),end=", " )
...
def bin_wise_bound_eval(fold_bounds_all_lms, fold_errors, fold_bins,landmarks, uncertainty_type, num_bins=5, show_fig=False):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        weighted_mean_lm = 0
        total_weights = 0
        for l in range(len(bins_sizes)):
            bin_acc = bins_acc[l]
            bin_size = bins_sizes[l]
...
def evaluate_jaccard(bin_predictions, uncertainty_pairs, num_bins, landmarks, num_folds=8, verbose=False):
    ...
    Args:
        ...
        verbose (bool): print statitics (default=False).
    ...
    for i, (model, data_structs) in enumerate(bin_predictions.items()):
        ...
        for up in uncertainty_pairs:
            ...
            if verbose:
                print("\n \n \n OVERALL JACC STATS FOR ", model + uncertainty_type)
                print("overall mean jacc: ", np.round(np.mean(fold_mean_landmarks),3), ' +/- ', np.round(np.std(fold_mean_landmarks),3))

                print("QUANTILE bin jaccs: ",end="")

                for idx_bin in range(num_bins):
                    print("B"+str(idx_bin), np.round(np.mean(fold_mean_bins[idx_bin]),3), '+/-', np.round(np.std(fold_mean_bins[idx_bin]),3),end=", " )
...
def bin_wise_jaccard(fold_errors, fold_bins, num_bins, landmarks, uncertainty_key, verbose=False):
    """ Helper function for evaluate_jaccard. 
        Args:
            ...
            verbose (bool)print statistics (default=False)
    ...
    for i, landmark in enumerate(landmarks):
        ...
        for bin in range(num_bins):
            ...
            gt_bins_errors = errors_groups[bin]
            ...
            if verbose:
                print("\n gt bin keys ", gt_bins_keys)
                print("gt bin errors ", gt_bins_errors)
                print("sim: ", j_sim)

                print("LM: ", landmark, " Bin: ", bin, " mean GT error: ", np.mean(gt_bins_errors), "mean pred error: ", np.mean(pred_b_errors), " and JAC SIM: ", j_sim)
        
        all_lm_jacc.append(np.mean(inner_jaccard_sims))

    if verbose:
        print("Overall mean jacc for all landmarks: ", np.mean(all_lm_jacc))
        print("Quantile mean jaccs ", [np.mean(x) for x in all_qs_jacc])

    return {"mean all lms": np.mean(all_lm_jacc), "mean all bins":  [np.mean(x) for x in all_qs_jacc], "all bins":  all_qs_jacc }
