...
def quantile_binning_and_est_errors(errors, uncertainties, num_bins, type='quantile', acceptable_thresh=5*0.9375):
    ...
    y_ = ir.fit_transform(uncertainties, errors )
    ...
    if type=="quantile":
        ...
        print("Acceptable error: ", acceptable_thresh)
        print("Error Boundaries: ", estimated_errors)
...
def box_plot(cmaps, landmark_uncert_dicts, uncertainty_types_list, models, x_axis_labels, x_label, y_label, num_bins, save_path=None, y_lim=120, verbose=False):
    ...
    Args:
        ...
        verbose (bool): whether to print statistics (default=False)m,
     

    """ 
    if verbose:
        print("MEASURING: ", y_label)
    ...
    for i, (up) in enumerate(uncertainty_types_list):
        ...
        for j in range(num_bins):
            ...
            for hash_idx, model_type in enumerate(models):
                ...
                if j==0:
                    if hash_idx == 1:
                        circ11 = patches.Patch( facecolor=cmaps[i],label= model_type + " " + uncertainty_type, hatch=hatch_type, edgecolor="black")
                    else:
                ...
                rect = ax.boxplot(percent_data, positions=x_loc,sym='', widths=width, showmeans=True, patch_artist=True)
...
def plot_cumulative(cmaps, data_struct, models, uncertainty_types, bins, save_path=None, verbose=False):
    ...
    Args:
        ...
        save_path (str):path to save plot to. If None, displays on screen (default=None),
        verbose (bool): whether to print statistics (default=False).
    """ 
    ...
    for i, (up) in enumerate(uncertainty_types):
        ...
        for hash_idx, model_type in enumerate(models):
            ...
            #Get key for datastruct

            # print("key = ", dict_key, model_type, uncertainty, data_struct.keys())
            #Filter inly the bins selected
            dataframe =  data_struct[model_type]
            # print(dataframe)
            model_un_errors = dataframe[dataframe[uncertainty + " Uncertainty bins"].isin(bins)][ uncertainty + " Error"].values
            ...
            ax.plot(sorted_errors, p, label=model_type + " " + uncertainty, color=colour ,  linestyle = line, dash_capstyle = 'round',)
