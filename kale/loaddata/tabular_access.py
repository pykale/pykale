...
def update_csvs_with_folds(datapath, all_fold_info_paths):
    ...
    if "PHD-NET" in datapath:
        # all ids
        all_ids = [
            d["id"] for d in val_test_splits["validation"] + val_test_splits["testing"] + val_test_splits["training"]
        ]
        for a_id in all_ids:
            uncert_er_datafame.loc[
                uncert_er_datafame["uid"].isin(
                    [a_id + "EHJ_220", a_id + "ASPIRE SSC_246", a_id + "EHJ_220", a_id + "ASPIRE SSC_120"]
                ),
                "uid",
            ] = a_id
