...
class VideoBiDomainDatasets(BiDomainDatasets):
    ...
    def get_domain_loaders(self, split="train", batch_size=32, num_workers=0):
        ...
        if self.rgb:
            ...
            rgb_source_loader = self._source_sampling_config.create_loader(
                rgb_source_ds,
        ...
        if self.flow:
            ...
            flow_source_loader = self._source_sampling_config.create_loader(
                flow_source_ds,
                batch_size=batch_size,
                num_workers=num_workers,
            )
        ...
        if self._labeled_target_by_split is None:
            ...
            if self.rgb:
                rgb_target_loader = self._target_sampling_config.create_loader(
                    rgb_target_ds,
                    batch_size=batch_size,
                    num_workers=num_workers,
                )
            ...
            if self.flow:
                flow_target_loader = self._target_sampling_config.create_loader(
                    flow_target_ds,
                    batch_size=batch_size,
                    num_workers=num_workers,
                )
            ...
            if self.rgb:
                ...
                rgb_target_labeled_loader = FixedSeedSamplingConfig(balance=True, class_weights=None).create_loader(
                    rgb_target_labeled_ds,
                    batch_size=min(len(rgb_target_labeled_ds), batch_size),
                    num_workers=num_workers,
                )

                rgb_target_unlabeled_loader = self._target_sampling_config.create_loader(
                    rgb_target_unlabeled_ds,
                    batch_size=batch_size,
                    num_workers=num_workers,
                )
            ...
            if self.flow:
                ...
                flow_target_labeled_loader = FixedSeedSamplingConfig(balance=True, class_weights=None).create_loader(
                    flow_target_labeled_ds,
                    batch_size=min(len(flow_target_labeled_ds), batch_size),
                    num_workers=num_workers,
                )
                flow_target_unlabeled_loader = self._target_sampling_config.create_loader(
                    flow_target_unlabeled_ds,
                    batch_size=batch_size,
                    num_workers=num_workers,
                )
                n_dataset = DatasetSizeType.get_size(
