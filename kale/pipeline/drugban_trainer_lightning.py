...
"""
This module contains the DrugBAN trainer class and related functions.

It trains a Interpretable bilinear attention network with or without domain
adaptation model for drug-target interaction prediction.
...
from kale.evaluate.metrics import binary_cross_entropy, cross_entropy_logits
...
class DrugbanTrainer(pl.LightningModule):
    ...
    Args:
        ...
        discriminator (torch.nn.Module): The domain discriminator model used for DA.
        config (dict): Configuration dictionary with settings for training, DA, and metrics.
    """

    def __init__(self, model, discriminator=None, **config):
        ...
        if self.da_use:
            ...
            if config["DA"]["RANDOM_LAYER"]:
                ...
                self.domain_dmm = discriminator(
                    input_size=config["DECODER"]["IN_DIM"] * self.n_class, n_class=self.n_class
                )
