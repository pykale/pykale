...
def negative_sampling(pos_edge_index: torch.Tensor, num_nodes: int) -> torch.Tensor:
    ...
    perm = torch.tensor(np.random.choice(num_nodes**2, idx.size(0)).tolist(), dtype=torch.int)
    mask = torch.isin(perm, idx)
    rest = mask.nonzero().view(-1)
    while rest.numel() > 0:  # pragma: no cover
        tmp = torch.tensor(np.random.choice(num_nodes**2, rest.size(0)).tolist(), dtype=torch.int)
