...
# Mock DGL graph creation (drug data)
def create_mock_dgl_graph(batch_size, node_feats_dim):
    # Create a simple DGL graph with 'batch_size' nodes
    graph = dgl.graph(([], []))
    graph.add_nodes(batch_size)

    # Create edges: In this example, connect each node to its next neighbor in a circular fashion
    src = torch.arange(batch_size)
    dst = torch.roll(src, shifts=-1)  # Shift the indices to create a circular connection
...
def test_drugban_forward_train():
    ...
    # Create mock inputs
    bg_d = create_mock_dgl_graph(batch_size, CONFIG["DRUG"]["NODE_IN_FEATS"])
...
def test_molecular_gcn_forward():
    ...
    # Create a mock DGL graph with node features
    bg = create_mock_dgl_graph(10, in_feats)
