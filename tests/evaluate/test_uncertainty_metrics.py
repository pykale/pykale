...
class TestCrossDataFrameIndexAlignment:
    ...
    def test_bin_wise_errors_misaligned_index(self):
        ...
        The reordered ``fold_bins`` keeps the same per-uid data as the aligned frame; only its
        row order and index differ. The result must be identical to the aligned case and match
        the hand-computed expectation.
        """
        errors_df = self._errors_df()
        bins_aligned = self._bins_df()
        # Same data, rows reversed and index reset -> label-aligned mask from fold_errors now
        # points at the wrong rows of fold_bins under the old cross-frame indexing.
        ...
        # Invariance: misaligned result equals the aligned result.
    ...
    def test_bin_wise_bound_eval_misaligned_index(self):
        """Bound accuracy must be independent of the index alignment between the two frames."""
        errors_df = self._errors_df()
        bins_aligned = self._bins_df()
        bins_misaligned = self._bins_df().iloc[[3, 2, 1, 0]].reset_index(drop=True)
        ...
        # With these bounds every error falls within its bin, so accuracy is perfect...
        assert aligned["mean all targets"] == pytest.approx(1.0)
        # ...and, crucially, the misaligned frame yields the same result.
