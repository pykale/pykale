...
class TestBatchSamplers:
    ...
    def test_balanced_small_batches(self):
        ...
        assert {self._idx_to_class(idx) for idx, nb in counts.items() if nb > 1} == {"1", "2", "4"}
    ...
    def test_balanced_bigger_batches(self):
        ...
        assert sorted(self._idx_to_class(idx) for idx, nb in counts.items() if nb > 1) == ["1", "1", "2", "2", "4", "4"]

    def test_reweighter(self):
        sampler = ReweightedBatchSampler(self.MyDataset(), batch_size=11, class_weights=np.array([1, 2, 1, 3]))
